...
def check_access(access, countries, acc):
    """
    Compares the access levels specified in the require_jwt decorator with the
    accesslevels specified in the given jwt. Returns a boolean stating whether
    there is a match.
    ...
    Args:
        ...
        countries ([str]) An optional list of countries for which each role
            ...
            If the length of countries is smaller than the length of access,
            then the final element of countries is repeated to make the length
            match. Accepts wildcard value "" for any country.  Default value is
            [""], meaning all specified access levels will be valid for any
            country if countires is not specified.
    ...
    for i in range(0, len(countries)):
        ...
        if country in acc:
            # ...and if corresponding country's role is specified in token...
        ...
        # ...Else if the country specified by the decorator is "" wildcard...
            ...
            for c in acc:
                # ...if an access level in jwt matches a level in arguments...
...
def get_token():
    ...
    we need to look for the token in multiple places depending on the type of
    request.
...
def check_auth(access, countries):
    """
    A function that checks whether the user is authorised to continue with the
    current request. It does this by verifying the jwt stored as a cookie. If
    the user isn't authorised the request is aborted with an Invalid Token
    Error.
    ...
    Args:
        ...
        countries ([str]) An optional list of countries for which each role
            title correspond to. access[0] corresponds to country[0] and etc...
            If the length of countries is smaller than the length of access,
            then the final element of countries is repeated to make the length
            match. Accepts wildcard value "" for any country.  Default value is
            [""], meaning all specified access levels will be valid for any
            ...
            Would give access to jordan managers, and shared accounts in demo.
    ...
    # Bable runs in Frontend not API - both import this module and musn't fail.
    ...
    try:
        ...
        # Get the user's data, directly if in auth module, or else remotely.
        try:
            from meerkat_auth.user import User
            user = User.from_db(payload['usr']).get_payload(payload['exp'])

        # Hacky: If we can't get data directly, then try a request to the url.
        ...
        # Check user has required access, and if so, store user details in g.
