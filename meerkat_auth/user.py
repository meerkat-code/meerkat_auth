...
class User:
    """
    Class to model a single User's account and manage writing, reading,
    ...
    def to_db(self):
        ...
        config['USERS']. First validates content.
    ...
    def get_access(self):
        """
        Returns an object detailing the complete list of roles this user has
        access to in each country.

        Returns:
            A dictionary where each key is a country and each value is a list
            of roles this user has access to in that country.
        """
        access = {}
    ...
    def get_jwt(self, exp):
        ...
        and the specified expiry time of the user's session.  This is
        specifically kept extremely small so it can be stored as a cookie or
        header.
    ...
    def get_user_jwt(self, exp):
        """
        Returns a large secure Json Web Token (JWT) giving all the users
        details. This is not intended for storing as a cookie or a header.
    ...
    def get_payload(self, exp):
        """
        Returns a dictionary giving all details for the user session
        If storing as a cookie or a header, params should be deleted
        as the payload is too large really.

        Args:
            exp (string) The expiry time of the user's session.
        Returns:
            The secure jwt.
        """
        return {
    ...
    def authenticate(username, password):
        """
        Checks whether the specified credentials are valid credentials for a
        user. If they are, returns the User object, if not throws an exception.
        ...
        Raises:
            InvalidCredentialException if any credentials are invalid.
        """
    ...
    def check_username(username):
        """
        Efficiently checks whether a specific username already exists in
        the database.
    ...
    def validate_username(username):
        """
        Checks whether the specified new username is valid or not, and raises
        an InvalidCredentialException if it is not valid.
    ...
    def validate_roles(countries, roles):
        """
        Checks whether the supplied roles and countries lists are valid
        parameters for creating a new user.  Crucially the roles must exist
        in the database and have a valid ancestor list.

        Args:
            countries ([str]) A list of strings where each stirng is the name
                of a country.
            roles ([str]) A list of strings where eachs tring is the title of
                a valid role.

        Raises:
            InvalidRoleException if there is a role that does not meet
                the criteria.
        """
    ...
    def update_user(username, email, unhashed_pass, countries, roles, data={}):
        ...
        Validates and prepares the data given, loads the user from db, edits
        the users data and writes the user to the db before returning the final
        user to the caller. Preparation includes the hashing of the new
        password.
        ...
        Raises:
            ...
            InvalidRoleException if a role doesn't exist or doesn't have a
            valid ancestor list.
        """

        # Hash the password with a salt size of 16 bytes and 29000 rounds.
    ...
    def get_all(countries, attributes):
        """
        Fetches from the database the requested attributes for all users that
        belong to the specified country. If country or attributes equate to
        false then all possible options for that argument will be used.
...
class InvalidCredentialException(Exception):
    """
    An exception to be raised when an invalid credential is supplied, typically
    username or password. This Exception prints out the credential in the error
    message, but only prints out the invalid value supplied if you called the
    repr() function on the object.
    """
    ...
    def __str__(self):
        ...
        msg = "Invalid credential supplied. Please check the {} and try again."
        return msg.format(
            self.credential
        )
    ...
    def __repr__(self):
        """
        String referencing both the invalid credential and the its invalid
        value.
        """
